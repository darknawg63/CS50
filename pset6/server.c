...
bool load(FILE* file, BYTE** content, size_t* length)
{
  ...
  // Read in the data char by char and append it to our buffer
  for (int ch = fgetc(file); ch != EOF; ch = fgetc(file))
    {
      data[i] = (char) ch;
      i++;

      // Reallocate buffer space and one extra for null terminator
      data = realloc(data, sizeof(char) * (i+1));
    }

  // Populate the string that was passed via pointer
  *content = data;
  *length = i;
...
bool parse(const char* line, char* abs_path, char* query)
{
  ...
  uri = malloc(strlen(line) * (sizeof(char) + 1));
  strcpy(uri, line);
  ...
  while( token != NULL )
    {
      // The method should be exclusively 'GET' and not accept abcGET
      if ((strcmp(token, "GET") == 0) && (strcmp(token, "GET  ") != 0) && i == 0)
        {
      ...
      if (token[0] == '/' && i == 1)
        {
          header = malloc(strlen(line) * (sizeof(char) + 1));
          strcpy(header, token);
        }
      else
      if (i == 1)
  ...
  // Eventually we check if there is a query string in the header
